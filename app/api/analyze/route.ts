...
export async function POST(req: NextRequest) {
  const { title, url, date, srcName, country, focus, customContext } = await req.json();

  const ctx = ENTITY_CONTEXT[country];
  if (!ctx) return NextResponse.json({ error: "Unknown country" }, { status: 400 });

  // 담당자가 편집한 맥락이 있으면 우선 사용, 없으면 기본값
  const businessContext = customContext || ctx.business;
  const focusNote = focus ? `\n\n이번 주 담당자 포커스: "${focus}"` : "";

  const prompt = `당신은 해외 신재생에너지 법인의 시장동향 분석 전문가입니다.

[법인 사업 맥락]
${businessContext}${focusNote}
...
- 제언은 법인 맥락의 구체적 프로젝트명·이슈와 연결, 2~3줄

JSON 형식으로만 응답 (마크다운 없이):
{
  "bullets": ["내용1", "내용2", "내용3", "내용4", "내용5", "내용6", "내용7"],
  "implication": "법인 관점 시사점 2~3줄. 구체적 프로젝트명·이슈와 연결. 보고체 사용."
}`;
